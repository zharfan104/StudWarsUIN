...
class UpdateInGame {
  static Future<Null> answerClickedSecond(
      {tipe,
      email,
      roomEmail,
      namaRoom,
      ...
      @required idxsoal,
      @required incr,
      @required jawab}) async {
    print("here answerClickedSecond $email");

    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'poin.$email': FieldValue.increment(poin)});
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'jawaban_soal.$email.${idxsoal + 1}': jawab});
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'soal.$email': FieldValue.increment(1)});
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
    ...
    if (incr) {
      await Firestore.instance
          .collection('$tipe')
          .document('$roomEmail $namaRoom')
          .updateData({'soal.game': FieldValue.increment(1)});
    }
  }

  static Future<Null> updateIndex({tipe, roomEmail, namaRoom}) async {
    print("updateIndex $namaRoom");
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'soal.game': FieldValue.increment(1)});
  ...
  static Future<Null> answerClickedFirst(
      {tipe,
      email,
      roomEmail,
      namaRoom,
      ...
      @required idxsoal,
      @required jawab}) async {
    print("here answerClickedFirst");
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'jawaban_soal.$email.$idxsoal': jawab});
    await Firestore.instance
        .collection('$tipe')
        .document('$roomEmail $namaRoom')
        .updateData({'poin.$email': FieldValue.increment(poin)});
