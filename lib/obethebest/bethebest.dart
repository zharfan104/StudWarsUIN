...
class _OneState extends State<BeT> {
  BeTBloc betBloc;
  ...
  @override
  void initState() {
    betBloc = BlocProvider.of<BeTBloc>(context);
    betBloc.dispatch(Start(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        // appBar: AppBar(title: Text('Flutter BeT')),
        body: BlocBuilder<BeTBloc, BeTState>(builder: (ctx, state) {
...
class Actions extends StatelessWidget {
  ...
  List<Widget> _mapStateToActionButtons({
    ...
    final BeTState state = betBloc.currentState;
    if (state is Ready) {
      return [
        FloatingActionButton(
          child: Icon(Icons.play_arrow),
          onPressed: () => betBloc.dispatch(Start(
              duration: state.duration,
              idxsoal: 3,
              lokal: state.lokal,
              isDone: false)),
    ...
    if (state is Running) {
      return [
        FloatingActionButton(
          ...
          onPressed: () => betBloc.dispatch(Pause()),
        ),
        FloatingActionButton(
          child: Icon(Icons.replay),
          onPressed: () => betBloc.dispatch(Reset()),
        ),
      ];
    }
    if (state is Paused) {
      return [
        FloatingActionButton(
          child: Icon(Icons.play_arrow),
          onPressed: () => betBloc.dispatch(Resume()),
        ),
        FloatingActionButton(
          child: Icon(Icons.replay),
          onPressed: () => betBloc.dispatch(Reset()),
