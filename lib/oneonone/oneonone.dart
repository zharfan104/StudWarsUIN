...
import 'bloc/one_event.dart';
import 'bloc/one_state.dart';
...
class _OneState extends State<One> {
  ...
  @override
  void initState() {
    oneBloc = BlocProvider.of<OneBloc>(context);
    oneBloc.dispatch(Start(
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        // appBar: AppBar(title: Text('Flutter One')),
        body: BlocBuilder<OneBloc, OneState>(builder: (ctx, state) {
...
class Actions extends StatelessWidget {
  ...
  List<Widget> _mapStateToActionButtons({
    ...
    final OneState state = oneBloc.currentState;
    if (state is Ready) {
      return [
        FloatingActionButton(
          child: Icon(Icons.play_arrow),
          onPressed: () => oneBloc.dispatch(Start(
              duration: state.duration,
              idxsoal: 3,
              lokal: state.lokal,
              isDone: false)),
    ...
    if (state is Running) {
      return [
        FloatingActionButton(
          ...
          onPressed: () => oneBloc.dispatch(Pause()),
        ),
        FloatingActionButton(
          child: Icon(Icons.replay),
          onPressed: () => oneBloc.dispatch(Reset()),
        ),
      ];
    }
    if (state is Paused) {
      return [
        FloatingActionButton(
          child: Icon(Icons.play_arrow),
          onPressed: () => oneBloc.dispatch(Resume()),
        ),
        FloatingActionButton(
          child: Icon(Icons.replay),
          onPressed: () => oneBloc.dispatch(Reset()),
